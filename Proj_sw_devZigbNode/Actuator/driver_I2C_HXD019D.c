...
#if(SWITCH_TYPE_FORCEDEF == SWITCH_TYPE_INFRARED) //强制设备类型为红外转发器时，此驱动生效
...
void thread_infraredSM(void){
	...
	if(!dev_agingCmd_sndInitative.agingCmd_infrareOpreat)IR_currentOpreatRes = 0; //若红外主动上发时效为0，则将红外当前操作结果数据值清零
	...
	switch(theadStatus_infrared){
		...
		case infraredSMStatus_sigSendSTBY:{ //控制就绪
			...
			if(!infraredAct_timeCounter)theadStatus_infrared = infraredSMStatus_sigSend; //控制就绪等待时间，等待硬件稳定
